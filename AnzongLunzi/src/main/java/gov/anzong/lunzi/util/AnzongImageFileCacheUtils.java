 ...
 * //   创建时间：2016-11-01 09:12
 * //   这玩意的用处：利用文件缓存图片
 */
public class AnzongImageFileCacheUtils {
    private static String PATH;
    private static final String CHCHEDIR = "ImageChace";//缓存目录
    private static final String WHOLESALE_CONV = ".cache";//缓存文件后缀名
    private static final int MB = 1024 * 1024;
    private static final int CACHE_SIZE = 120;//缓存最大容量（超过就会利用lru算法删除最近最少使用的缓存文件）
    private static final int FREE_SD_SPACE_NEEDED_TO_CACHE = 100;//缓存所需SD卡所剩的最小容量

    private static AnzongImageFileCacheUtils instance = null;

    //单例模式
    public static AnzongImageFileCacheUtils getInstance(String path) {
        if (instance == null) {
            synchronized (AnzongImageFileCacheUtils.class) {
                if (instance == null) {
                    instance = new AnzongImageFileCacheUtils(path);
                }
            }
        }
        return instance;
    ...
    AnzongImageFileCacheUtils(String path) {
        PATH = path;
    ...
    private String convertUrlToFileName(String url) {
        String fileName = AnzongMD5Utils.getMD5(url);
        if (!AnzongStringUtils.isBlank(fileName)) {
